...
def accuracy(y_hat, y):
    ...
    accuracy_ = float(reduce_sum(astype(cmp, y.dtype)))
    return accuracy_
...
def train_ch13(net, 
               ...
               loss, trainer, 
               num_epochs, devices=try_all_gpus()):
    """
        Train a model with mutiple GPUs (defined in Chapter 13).
    """
    timer = Timer()
    num_batches = len(train_iter)
    # print("start train, num_batches is %s" % num_batches)
