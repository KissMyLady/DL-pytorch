...
class BERTModel(nn.Module):
    ...
    def __init__(self, vocab_size, 
        ...
        self.nsp = NextSentencePred(nsp_in_features)  # nsp_Y_hat
...
def get_BERT_model(len_vocab=20256):
    ...
    """
    原始BERT :cite:Devlin.Chang.Lee.ea.2018有两个不同模型尺寸的版本。

    - 基本模型（ BERTBASE）
        使用12层（Transformer编码器块），768个隐藏单元（隐藏大小）和12个自注意头。
    - 大模型（ BERTLARGE）
        使用24层，1024个隐藏单元和16个自注意头。
